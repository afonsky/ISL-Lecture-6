<template>
  <footer
    ...
    class="absolute bottom-0 text-center left-0 right-0 px-2 py-0 text-xs"
      >
      <button @click="$slidev.nav.openInEditor()" title="Open in AAA Editor" class="icon-btn opacity-20 !border-none !hover:text-white p-0">
          <carbon:edit />
      </button>
      {{ (conf('metaFooter') ?? "date date morefooter").split(" ").map(k => $slidev.configs[k]).filter(v => !!v).join(" − ") }}
      −
